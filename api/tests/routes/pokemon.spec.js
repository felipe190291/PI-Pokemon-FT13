...
describe("GET /pokemons", function () {
  it("cuantos pokemon hay en mi inicio", () => {
    return agent
      ...
      .expect(200)
      .then((res) => {
        expect(res.body.length).to.be.deep.equal(12);
      });
  });
});
...
describe("GET /pokemons/:id", () => {
  it("responds with  a id message, Lo sentimos no se encontro tu pokemon", () => {
    return agent
      .get("/pokemons/1?limit=10")
      .expect(200)
      .expect((res) => {
        console.log("holaaaa", res.body.name);
        expect(res.body.name).to.be.deep.equal("bulbasaur");
      });
  });
});
